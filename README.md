...
- **Triggers**: Two Cloud Run services to launch the pipeline:
  ...
  - **Insights Trigger**: Fires when CCAI Insights publishes a conversation notification.
...
1.  **Direct Upload Workflow (GCS Trigger)**: Use this if you want files redacted *immediately* upon upload, before or independent of CCAI Insights.
...
*   **Option B (Insights)**: Redacts files *only* when CCAI Insights processes them. Best for Contact Center integration.
...
Deploy the service that listens to CCAI Insights conversation events (via Pub/Sub).
